...
function setHoverToResetButton() {
    // Add a mouseover event listener
    resetButton.addEventListener('mouseover', () => {
        // Change the resetButton's background color
        resetButton.style.backgroundColor = '#9FE8DF';
    });

    // Add a mouseout event listener
    resetButton.addEventListener('mouseout', () => {
        // Change the resetButton's background color back to its original color
    ...
    tipList.forEach(button => {
        ...
        button.addEventListener('click', (event) => {
            ...
            for (const key in tips) {
                ...
                if (key == button.id) {
                    setActiveClassToTipButton(button);
                    inputCustom.value = tips[key];
                    validateForm();
                    break
                }
...
function shouldReplaceCustomTip() {
...
function setActiveClassToTipButton(button) {
